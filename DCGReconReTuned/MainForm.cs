using DCGReconReTuned.Classes;
using DCGReconReTuned.Forms;
...
namespace DCGReconReTuned
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("It appears that you have yet to configure your receiver list.\n\nIn order to continue, please add atleast one receiver to monitor.", "DCG Recon", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                IsFirstTime = true;
        ...
        private async void Tuner1SetChannelButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"Channel {Tuner1ChannelTextbox.Text} does not exist. Please try another channel.", "DCG Recon", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (response == null)
            {
                MessageBox.Show($"Channel {Tuner1ChannelTextbox.Text} exists, but is not authorized at this time. Please try another channel.", "DCG Recon", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private async void Tuner2SetChannelButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"Channel {Tuner2ChannelTextbox.Text} does not exist. Please try another channel.", "DCG Recon", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (response == null)
            {
                MessageBox.Show($"Channel {Tuner2ChannelTextbox.Text} exists, but is not authorized at this time. Please try another channel.", "DCG Recon", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            toolTip1.ToolTipTitle = "Channel Set";
            toolTip1.ToolTipIcon = ToolTipIcon.Info;
        ...
        {
            if (MessageBox.Show("Are you sure you would like to Auto-Assign a channel? This can take a while.\n\nNote: You will not be able to continue until it is complete.", "DCG Recon", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
            {
                ...
                if (WasChannelChanged)
                {
                    ...
                    MessageBox.Show($"Auto-Assigned channel {channel} to Tuner 1, {Receiver.ReceiverName}", "DCG Recon", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    LoadingProgressBar.Value = filtered.Count();
                    MessageBox.Show($"No channel available to assign. Please try again later.", "DCG Recon", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        private async void Tuner2AutoAssignButton_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Are you sure you would like to Auto-Assign a channel? This can take a while.\n\nNote: You will not be able to continue until it is complete.", "DCG Recon", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
            {
                ...
                {
                    LoadingProgressBar.Value = filtered.Count();
                    MessageBox.Show($"Auto-Assigned channel {channel} to Tuner 2, {Receiver.ReceiverName}", "DCG Recon", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    LoadingProgressBar.Value = filtered.Count();
